      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold text-slate-800 dark:text-white">
            Nos <span className="text-[#008080]">Domaines d'Excellence</span>
          </h2>
          <p className="mt-4 text-lg text-slate-600 dark:text-slate-300">
            Chaque expertise est une spécialisation profonde — stack, KPIs et cas clients à l'appui.
          </p>
        </motion.div>
          ...
          {expertises.map((exp, index) => {
            ...
            return (
              ...
              >
                <AccordionItem
                  value={exp.id}
                  className={`border ${exp.borderColor} rounded-2xl px-6 overflow-hidden bg-white dark:bg-slate-800/50`}
                >
                  <AccordionTrigger className="hover:no-underline py-6">
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-xl ${exp.bg} flex-shrink-0`}>
                        <Icon className={`w-6 h-6 ${exp.color}`} />
                      </div>
                      <span className="text-xl font-bold text-slate-800 dark:text-white text-left">
                        {exp.title}
                      </span>
                    </div>
                  </AccordionTrigger>
                  <AccordionContent className="pb-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-6">
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {exp.stack.map((tech) => (
                            <span
                              key={tech}
                              className="px-3 py-1 bg-[#008080]/10 text-[#008080] text-xs font-semibold rounded-full"
                            >
                              {tech}
                            </span>
                          ))}
                      ...
                      <div>
                        ...
                        <ul className="space-y-1.5">
                          {exp.kpis.map((kpi) => (
                            <li key={kpi} className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-4 border-l-4 border-[#008080]">
                      <p className="text-xs font-bold uppercase tracking-widest text-[#008080] mb-1">
                        Cas client
                      </p>
